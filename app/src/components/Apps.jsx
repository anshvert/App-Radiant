...
function Apps() {
  ...
  useEffect(()=>{
    const getAllApps = async () => {
      ...
      const categorizedApps = allApps.data.reduce((acc, curr) => {
        ...
        <div className="row">
          ...
          <div className="app-cards">
            {dailyApps.map((app, index) => (
              <AppCard key={index} app={app} onClick={() => navigate(`${location.pathname}/${app.url}`)}/>
            ))}
          </div>
        </div>
        <div className="row">
          ...
          <div className="app-cards">
            {games.map((app, index) => (
              <AppCard key={index} app={app} onClick={() => window.open(app.url, "_blank")}/>
            ))}
          </div>
        </div>
      </div>
